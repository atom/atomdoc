...
const VisibilityRegex = '^\\s*([a-zA-Z]+):\\s*';
const ReturnsRegex = `(${VisibilityRegex})?\\s*Returns`;
...
var parseReturnValues = function(tokens, consumeTokensAfterReturn) {
  ...
  const returnsMatches = new RegExp(ReturnsRegex).exec(firstToken.text); // there might be a `Public: ` in front of the return.
...
var parseListItem = function(argumentString) {
  ...
  const nameMatches = new RegExp(ArgumentListItemRegex).exec(argumentString)
  if (nameMatches) {
    name = nameMatches[1];
...
const isReturnValue = string => new RegExp(ReturnsRegex).test(string);

const isArgumentListItem = string => new RegExp(ArgumentListItemRegex).test(string);
...
const stopOnSectionBoundaries = function(token, tokens) {
  if (['paragraph', 'text'].includes(token.type)) {
    ...
    if ((listToken) && new RegExp(ArgumentListItemRegex).test(listToken.text)) {
      return false;
    }
  }
    ...
    switch (token.type) {
      ...
      case 'text': case 'code': case 'blockquote_start':
        ...
        for (let line of textLines) {
          const prefix = linePrefix ? linePrefix : `${indent()}  `;
